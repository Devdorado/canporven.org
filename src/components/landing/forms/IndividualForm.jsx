...
const helpOptions = [
  { value: 'donate', key: 'individual.help_donate' },
  { value: 'skills_volunteering', key: 'individual.help_skills' },
  { value: 'spread_word', key: 'individual.help_spread' },
  { value: 'other', key: 'individual.help_other' },
];
...
export default function IndividualForm() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setSubmitting(true);
    await base44.entities.IndividualRegistration.create(form);
    setSubmitting(false);
    setSubmitted(true);
  };
  ...
  if (submitted) {
    return (
      <div className="max-w-xl mx-auto text-center bg-white rounded-2xl border border-[#1565C0]/20 shadow-sm p-8">
        ...
        <p className="text-[#121212]/70">{t('getinvolved.success_individual', lang)}</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div>
        <label className="block text-sm font-medium text-[#121212] mb-1.5">{t('individual.name', lang)}</label>
        <input
          type="text"
          required
          value={form.name}
      ...
      <div>
        <label className="block text-sm font-medium text-[#121212] mb-2">{t('individual.how_help', lang)}</label>
      ...
      <div>
        <label className="block text-sm font-medium text-[#121212] mb-1.5">{t('individual.message', lang)}</label>
        <textarea
          value={form.message}
      ...
      <div className="flex items-start gap-3 pt-1">
        ...
        />
        <label htmlFor="individual-consent-gi" className="text-sm text-[#121212]/80 leading-snug cursor-pointer">
          {t('gi.consent', lang)} *
        </label>
      </div>
      ...
      <button
        type="submit"
        disabled={submitting || !form.gdpr_consent || form.help_ways.length === 0}
